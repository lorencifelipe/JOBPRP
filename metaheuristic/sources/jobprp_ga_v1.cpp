...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        cout << lytFileName << endl;
        cout << ordFileName << endl;
        cout << n << endl;
        cout << capacityLimit << endl;
        cout << SEED << endl;
        cout << POP_SIZE << endl;
        cout << OFFSPRING_SIZE << endl;
        cout << MUTATION_PROB << endl;
        for (int i : mu)
        {
            cout << i << " ";
        }
        cout << endl;
        cout << best.cromo.size() << endl;
        // Print orders
        for (int g = 0; g < best.cromo.size(); g++)
        {
            for (int o : best.cromo[g].orders)
            {
                cout << o << " ";
            }
            cout << endl;
        }
        // Print distance of each order
        for (int g = 0; g < best.cromo.size(); g++)
        {
            cout << best.cromo[g].distance << endl;
        }
        // Routes
        for (int g = 0; g < best.cromo.size(); g++)
        {
            vector<vector<int>> locations = returnLocations(best.cromo[g].orders, items);
            solution = traceRoute(numPickingPos+1, 5, numAisles, locations);
            opt = optTour(solution);
            optOrder = traceTour(locations, opt, depot, numAisles, numPickingPos+1);
            for (coord c : optOrder)
            {
                cout << "[" << c.aisle << "][" << c.location << "] ";
            }
            cout << endl;
        }
    }
